...
const galleryRecord = (
  ...
  items: alts.map((alt): $Typed<app.bsky.embed.gallery.Image> => ({
...
const galleryView = (alts: string[]): $Typed<app.bsky.embed.gallery.View> => ({
  $type: 'app.bsky.embed.gallery#view',
  items: alts.map((alt): $Typed<app.bsky.embed.gallery.ViewImage> => ({
...
describe(`matchMuteWords`, () => {
  ...
  describe(`gallery embed alt text`, () => {
    ...
    it(`matches alt text inside a quoted record's gallery`, () => {
      const quoted: app.bsky.feed.post.Main = mock.post({
        text: 'inside quoted',
    ...
    it(`matches alt text on a quoted record's recordWithMedia gallery`, () => {
      const quotedRecord: app.bsky.feed.post.Main = mock.post({
        ...
        embed: {
          ...
          } satisfies $Typed<app.bsky.embed.record.Main>,
          media: galleryRecord(['contains badword in nested gallery']),
        } satisfies $Typed<app.bsky.embed.recordWithMedia.Main>,
      })
      const res = moderatePost(
        mock.postView({
          record: mock.post({ text: 'outer post' }),
    ...
    it(`matches alt text on the view-side recordWithMedia gallery`, () => {
      const recordWithMediaView: $Typed<app.bsky.embed.recordWithMedia.View> = {
